/* ==========================================================================
   ...
   ========================================================================== */
let WORLD_W=7200, WORLD_H=5280;   // ingrandita per i tile da 120px: stessi palazzi, prospettiva piu' realistica
...
function matchCfg(){ if(GAME.match==='blitz') return {total:30,w:5760,h:4200,loot:130,deploy:5,first:16000,wait:11000,shrink:11000,pr:[0.60,0.40,0.24,0.12,0.05],pd:[2,3,5,8,13]};
  ...
  return {total:100,w:7200,h:5280,loot:200,deploy:8,first:30000,wait:18000,shrink:16000,pr:[0.80,0.64,0.50,0.36,0.24,0.13,0.05],pd:[1,1,2,3,5,8,12]}; }
...
class Splash extends Phaser.Scene{
  ...
  begin(){
    ...
    if(this.cache.video && this.cache.video.exists('intro_video')){
      ...
      this.add.text(cx,H-30,'tocca per saltare',{fontFamily:TITLE_FONT,fontSize:'13px',color:'#7fa79b',fontStyle:'900'}).setOrigin(0.5).setDepth(6).setAlpha(0.7);
...
class Tutorial extends Phaser.Scene{
  ...
  create(){
    ...
    this.skip=this.add.text(W-14,26,'SALTA \u203a',{fontFamily:UI.MONO,fontSize:'13px',color:UI.faint})
  ...
  show(i){
    ...
    let y=pan.top+8;
    this.els.push(this.add.text(ix,y,s.h,{fontFamily:TITLE_FONT,fontSize:'26px',fontStyle:'700',color:UI.ink,
      wordWrap:{width:iw}}).setOrigin(0,0).setDepth(5));
    y+=this.els[this.els.length-1].height+10;
    this.els.push(this.add.text(ix,y,s.p,{fontFamily:TITLE_FONT,fontSize:'16px',color:UI.dim,
      lineSpacing:8,wordWrap:{width:iw}}).setOrigin(0,0).setDepth(5));
    ...
    this.hintTxt=this.add.text(cx,this.py+this.ph-22,s.hint,{fontFamily:UI.MONO,fontSize:'14px',color:UI.faint})
  ...
  buildDemo(kind,x,y,w,h){
    ...
    if(kind==='tap'){
      ...
      [[cx-70,'\u2726','TATTICA',C.player],[cx+70,'\u2605','ULTIMATE',C.gold]].forEach((b,i2)=>{
        ...
        push(this.add.text(b[0],cy+48,b[2],{fontFamily:UI.MONO,fontSize:'13px',color:UI.faint}).setOrigin(0.5).setDepth(6));
...
function uiPanel(sc,x,y,w,h,title,col,depth){
  ...
  if(title){ out.push(sc.add.text(x+w/2,y+ch*0.55,title,{fontFamily:TITLE_FONT,fontSize:'14px',
...
function uiSwitch(sc,x,y,opt){
  ...
  const tTitle=sc.add.text(x+17,y+h/2-13,opt.title,{fontFamily:TITLE_FONT,fontSize:'18px',fontStyle:'700'}).setOrigin(0,0.5).setDepth(d+1);
  const tSub=sc.add.text(x+17,y+h/2+15,opt.sub||'',{fontFamily:UI.MONO,fontSize:'13px'}).setOrigin(0,0.5).setDepth(d+1);
...
function uiCta(sc,x,y,w,h,label,col,depth,onTap){
  ...
  const t=sc.add.text(x+w/2,y+h/2,label,{fontFamily:TITLE_FONT,fontSize:'18px',fontStyle:'700',color:'#04120c'}).setOrigin(0.5).setDepth(d+1);
...
function uiCtaGhost(sc,x,y,w,h,label,col,depth,onTap){
  ...
  const t=sc.add.text(x+w/2,y+h/2,label,{fontFamily:TITLE_FONT,fontSize:'17px',fontStyle:'700',color:hexStr(c)}).setOrigin(0.5).setDepth(d+1);
  const z=sc.add.rectangle(x+w/2,y+h/2,w,h,0xffffff,0.001).setDepth(d+2).setInteractive({useHandCursor:true});
...
function uiStat(sc,x,y,w,h,label,value,col,depth){
  ...
  return [g,
    sc.add.text(x+10,y+13,label,{fontFamily:UI.MONO,fontSize:'13px',color:UI.faint}).setOrigin(0,0.5).setDepth(d+1),
    sc.add.text(x+10,y+h-17,''+value,{fontFamily:UI.MONO,fontSize:'20px',color:hexStr(c)}).setOrigin(0,0.5).setDepth(d+1)];
}
// tag di sistema mono
function uiTag(sc,x,y,txt,depth){ return sc.add.text(x,y,txt,{fontFamily:UI.MONO,fontSize:'14px',color:UI.faint})
...
function cyberBtn(sc,x,y,w,h,col,label,sub,cb,depth){
  ...
  if(sub) sc.add.text(x,y+12,sub,{fontSize:'13px',color:'#7fa79b'}).setOrigin(0.5).setDepth(d+1);
...
function overlayPanel(sc,title,build){ const W=sc.scale.width,H=sc.scale.height,cx=W/2; const els=[];
  ...
  els.push(uiHazard(sc,px+24,pan.top+4,pw-48,C.magenta,402));
  const add=(o)=>{ o.setDepth(402); els.push(o); return o; };
  build(cx,pan.top+26,pw,add);
  const cw=Math.min(220,W*0.62);
  const close=uiCta(sc,cx-cw/2,py+ph-64,cw,48,'CHIUDI',C.player,402,()=>{ els.forEach(o=>o.destroy&&o.destroy()); });
  close.els.forEach(o=>els.push(o));
}
...
class Menu extends Phaser.Scene{
  ...
  create(data){
    ...
    this.credTxt=this.add.text(W-16,26,'',{fontFamily:UI.MONO,fontSize:'14px',color:'#ffc247',padding:{top:4,bottom:2}}).setOrigin(1,0.5).setDepth(5);

    // ---- tre tessere statistiche ----
    const pad=14, sw3=Math.floor((Math.min(W,420)-pad*2-14)/3), sx0=cx-(sw3*3+14)/2, syy=H*0.185;
    uiStat(this,sx0,           syy,sw3,64,'VITTORIE',d.wins, C.gold,1);
    uiStat(this,sx0+sw3+7,     syy,sw3,64,'KILL',    d.kills,C.magenta,1);
    uiStat(this,sx0+(sw3+7)*2, syy,sw3,64,'WINRATE', wr+'%', C.player,1);

    // ---- layout calcolato DAL FONDO: le due azioni devono stare sempre
    //      sopra la barra delle icone, su qualunque altezza di schermo ----
    const navH=64;                                  // barra icone in basso (vedi barH sotto)
    const ghH=44, ctaH=54, gapA=9;
    const ghY = H-navH-16-ghH;                      // SFIDA UN AMICO
    const ctaY= ghY-gapA-ctaH;                      // INIZIA
    const pw=Math.min(352,W*0.92), px=cx-pw/2;
    const py=H*0.185+72;
    const ph=Math.max(210,(ctaY-14)-py);            // il pannello riempie quello che resta

    const pan=uiPanel(this,px,py,pw,ph,'CONFIGURAZIONE',C.player,0);
    const iw=(pw-46)/2, ix=px+22, gap=iw+8;
    // altezza dei banner ricavata dallo spazio reale del pannello
    ...
    const abw=Math.min(340,W*0.88);
    uiCta(this,cx-abw/2,ctaY,abw,ctaH,'\u25b6  INIZIA',C.player,2,()=>this.scene.start('Loadout'));
    uiCtaGhost(this,cx-abw/2,ghY,abw,ghH,'\u2694  SFIDA UN AMICO',C.magenta,2,()=>this.openChallenge());
  ...
  openChallenge(){ const W=this.scale.width,H=this.scale.height,cx=W/2; const els=[]; const E=o=>{els.push(o);return o;};
    ...
    if(hist.length){ E(this.add.text(cx,py+ph-150,'— STORICO SFIDE —',{fontFamily:TITLE_FONT,fontSize:'13px',color:'#7fa79b',fontStyle:'900'}).setOrigin(0.5).setDepth(402));
      hist.slice(0,4).forEach((h,i)=>{ const yy=py+ph-132+i*24;
        ...
        E(this.add.text(cx-pw/2+22,yy,'vs '+(h.vs.who||'?'),{fontSize:'13px',color:'#b9d8ce'}).setOrigin(0,0.5).setDepth(402));
        E(this.add.text(cx+pw/2-22,yy,res,{fontFamily:TITLE_FONT,fontSize:'13px',color:col,fontStyle:'900'}).setOrigin(1,0.5).setDepth(402));
      });
    }
  ...
  showVerdict(w,mine,their){ const W=this.scale.width,H=this.scale.height,cx=W/2; const els=[];
    ...
    rows.forEach((r,i)=>{ const yy=H*0.45+i*30;
      els.push(this.add.text(cx,yy,r[0],{fontSize:'13px',color:'#7fa79b'}).setOrigin(0.5).setDepth(502));
  ...
  openTransfer(){ const W=this.scale.width,H=this.scale.height,cx=W/2; const els=[]; const E=o=>{els.push(o);return o;};
    ...
    const avail=E(this.add.text(cx,py+150,'',{fontSize:'13px',color:'#7fa79b'}).setOrigin(0.5).setDepth(402));
    ...
    const vTxt=E(this.add.text(cx,vy+24,ready?('🎁 GENERA BUONO '+Math.floor(sp/10000)+'€'):('🎁 Buono studio a '+STUDIO_REWARD.cost+' ('+sp+')'),
      {fontFamily:TITLE_FONT,fontSize:'13px',color:ready?'#5df08a':'#6d8a80',fontStyle:'900',padding:{top:6,bottom:2}}).setOrigin(0.5).setDepth(403));
    ...
    E(this.add.text(cx,py+ph-84,'💾 SALVA PROGRESSI SU INKANIMUS',{fontFamily:TITLE_FONT,fontSize:'13px',color:'#5df08a',fontStyle:'900',padding:{top:6,bottom:2}}).setOrigin(0.5).setDepth(403));
    ...
    E(this.add.text(cx,py+ph-130,'♻ RIPRISTINA DA CODICE (NXS1:...)',{fontFamily:TITLE_FONT,fontSize:'13px',color:'#b9d8ce',fontStyle:'900'}).setOrigin(0.5).setDepth(403));
    ...
    E(this.add.text(cx,py+ph-34,'CHIUDI',{fontFamily:TITLE_FONT,fontSize:'13px',color:'#3df2b4',fontStyle:'900'}).setOrigin(0.5).setDepth(406));
    ...
    const drawList=(title,list,col,yStart)=>{
      ...
      let yy=yStart+22;
      list.forEach(c=>{
        E(this.add.text(cx-pw/2+20,yy,c.t,{fontSize:'13px',color:c.done?'#5df08a':'#e6fbf3',fontStyle:'800'}).setOrigin(0,0.5).setDepth(402));
        E(this.add.text(cx+pw/2-20,yy,(c.done?'✓ ':'')+'+'+c.reward,{fontFamily:TITLE_FONT,fontSize:'13px',color:'#ffc247',fontStyle:'900',padding:{top:6,bottom:2}}).setOrigin(1,0.5).setDepth(402));
    ...
    E(this.add.text(cx,y+14,'🎁 BUONO STUDIO',{fontFamily:TITLE_FONT,fontSize:'13px',color:ready?'#5df08a':'#ff3355',fontStyle:'900',padding:{top:6,bottom:2}}).setOrigin(0.5).setDepth(403));
  ...
  openProfile(){
    ...
    E(this.add.text(cx-pw/2+96,py+34,'OPERATORE',{fontFamily:TITLE_FONT,fontSize:'12px',color:'#7fa79b',fontStyle:'900'}).setOrigin(0,0.5).setDepth(402));
    ...
    E(this.add.text(cx-pw/2+96,py+70,(op.role||'')+' · Skin '+SKIN(GAME.skin).name,{fontSize:'13px',color:'#7fa79b'}).setOrigin(0,0.5).setDepth(402));
    // wallet chips row
    ...
    cards.forEach((c,i)=>{ const r=Math.floor(i/cols), col=i%cols;
      ...
      cont.add(g);
      cont.add(this.add.text(x+12,yy+13,c[0],{fontFamily:TITLE_FONT,fontSize:'12px',color:'#7fa79b',fontStyle:'900'}).setOrigin(0,0.5));
  ...
  openSettings(){
    ...
    qs.forEach((q,i)=>{ const bw=(pw-52)/3, x=cx+(i-1)*(bw+6);
      ...
      E(this.add.text(x,yy-11,q[1],{fontFamily:TITLE_FONT,fontSize:'14px',color:'#e6fbf3',fontStyle:'900'}).setOrigin(0.5).setDepth(403));
      E(this.add.text(x,yy+12,q[2],{fontSize:'12px',color:'#93b8ad'}).setOrigin(0.5).setDepth(403));
    ...
    E(this.add.text(cx,yy+22,'Se il ROYALE a 100 scatta, scegli BASSI: riduce particelle, bagliori e scosse camera.',
      {fontSize:'13px',color:'#b9d8ce',align:'center',wordWrap:{width:pw-56},lineSpacing:3}).setOrigin(0.5).setDepth(402));
    yy+=64;

    // --- OPACITÀ MINIMAPPA ---
    E(this.add.text(cx,yy,'OPACITÀ MINIMAPPA',{fontFamily:TITLE_FONT,fontSize:'13px',color:'#3df2b4',fontStyle:'900'}).setOrigin(0.5).setDepth(402));
    yy+=30;
    ...
    const pct=E(this.add.text(cx,yy+22,Math.round(GAME.mmAlpha*100)+'%',{fontFamily:TITLE_FONT,fontSize:'13px',color:'#e6fbf3',fontStyle:'900'}).setOrigin(0.5).setDepth(402));
    ...
    const aTxt=E(this.add.text(cx-half/2-3,yy,SFX.on?'🔊 AUDIO ON':'🔇 AUDIO OFF',{fontFamily:TITLE_FONT,fontSize:'13px',color:'#3df2b4',fontStyle:'900'}).setOrigin(0.5).setDepth(403));
...
class Game extends Phaser.Scene{
  ...
  enterDeploy(){
    ...
    this.dUI.push(this.add.text(W/2,70,'tocca la mappa per scegliere la zona',{fontSize:'13px',color:'#7fa79b'}).setOrigin(0.5).setScrollFactor(0).setDepth(62));
    ...
    this.countTxt=this.add.text(W/2,H-92,'',{fontSize:'14px',color:'#ff8a9c',fontStyle:'800'}).setOrigin(0.5).setScrollFactor(0).setDepth(62);
  ...
  // MASSA CHIUSA: palazzi accostati su griglia 120, non calpestabili, con un vicolo
  closedMass(x,y,w,h,tint){
    const T=120, cw2=Math.max(1,Math.round(w/T)), ch2=Math.max(1,Math.round(h/T));
    ...
    for(let c=0;c<cw2;c++)for(let r=0;r<ch2;r++){
      ...
      if(this.walls){ const b=this.walls.create(bx+T/2,by+T/2,'px').setVisible(false);
        b.setDisplaySize(T,T); b.refreshBody();
        this.wallRects.push({x:bx,y:by,w:T,h:T,type:'building'}); }
    }
  }
  buildWallRing(x,y,w,h,floorKey){
    ...
    if(cols>=12 && rows>=10){
      ...
      const wall=(wx,wy,ww,wh)=>{ g.fillStyle(IN,1); g.fillRect(wx,wy,ww,wh);
        if(this.walls){ const b2=this.walls.create(wx+ww/2,wy+wh/2,'px').setVisible(false);
          b2.setDisplaySize(ww,wh); b2.refreshBody();
          this.wallRects.push({x:wx,y:wy,w:ww,h:wh,type:'building'}); } };
      // taglio principale sul lato lungo, con una PORTA larga
      const vert=(x1-x0)>=(y1-y0);
      const DOOR=Math.round(T*0.8);
      if(vert){
        const cx=x0+Math.round((x1-x0)*Phaser.Math.FloatBetween(0.42,0.58));
        const dy=y0+Phaser.Math.Between(0,Math.max(0,(y1-y0)-DOOR));
        wall(cx,y0,WTH,dy-y0); wall(cx,dy+DOOR,WTH,y1-(dy+DOOR));
        // una stanza piu' piccola (bagno/cucina) in un angolo della meta' destra
        if((x1-cx)>T*2.4 && (y1-y0)>T*2.4){
          const ry=y0+Math.round((y1-y0)*0.55);
          const dx2=cx+Phaser.Math.Between(Math.round(T*0.6),Math.max(1,(x1-cx)-DOOR-10));
          wall(cx,ry,dx2-cx,WTH); wall(dx2+DOOR,ry,x1-(dx2+DOOR),WTH);
        }
      } else {
        const cy=y0+Math.round((y1-y0)*Phaser.Math.FloatBetween(0.42,0.58));
        const dx=x0+Phaser.Math.Between(0,Math.max(0,(x1-x0)-DOOR));
        wall(x0,cy,dx-x0,WTH); wall(dx+DOOR,cy,x1-(dx+DOOR),WTH);
        if((y1-cy)>T*2.4 && (x1-x0)>T*2.4){
          const rx=x0+Math.round((x1-x0)*0.55);
          const dy2=cy+Phaser.Math.Between(Math.round(T*0.6),Math.max(1,(y1-cy)-DOOR-10));
          wall(rx,cy,WTH,dy2-cy); wall(rx,dy2+DOOR,WTH,y1-(dy2+DOOR));
        }
      }
    }
  ...
  // ==========================================================================
  // PIANTA v2 — NIENTE citta' intera: la mappa e' fatta di 3 SETTORI, uno GRANDE
  // e due PICCOLI, ognuno con la sua ambientazione. Cosi' ogni partita e' diversa
  // e ogni tipo di tile ha lo spazio per essere sfruttato davvero.
  // ==========================================================================
  // ==========================================================================
  // PIANTA v3 — NIENTE griglia di isolati. La mappa e' TRE GRANDI SUPERFICI:
  //   3/7 in alto  = settore GRANDE (ospedale intero, o parco col lago)
  //   2/7 + 2/7 in basso = due settori PICCOLI (case, negozi, parco)
  // Strade: UNA orizzontale che attraversa tutto + UNA verticale che scende dal
  // centro fino al bordo. Un solo incrocio a T. Dentro i settori NON ci sono strade.
  // ==========================================================================
  // Riempie i tre settori: edifici GRANDI appoggiati dentro la superficie,
  // non isolati ritagliati da una griglia. Fra un edificio e l'altro resta il
  // passaggio: e' quello lo spazio in cui si cammina.
  fillSectors(){
    const P=this.city, T=P.TS;
    if(!P||!P.areas) return;
    const px=t=>t*T;
    P.areas.forEach(A=>{
      const X=px(A.tx), Y=px(A.ty), Wp=px(A.tw), Hp=px(A.th);
      // fondo del settore
      const bg = A.kind==='parco' ? (this.textures.exists('gr_grass')?'gr_grass':'pz_floor') : 'pz_floor';
      if(this.textures.exists(bg)) this.add.tileSprite(X,Y,Wp,Hp,bg).setOrigin(0,0).setDepth(-18.4);

      if(A.kind==='parco'){ this.fillPark(A); return; }
      if(A.kind==='ospedale'){ this.fillBigVenue(A,'ospedale'); return; }
      if(A.kind==='uffici'){ this.fillBigVenue(A,'ufficio'); return; }
      this.fillTown(A);                      // residenza / negozio
    });
  }
  // PARCO: lago (gia' nella pianta), alberi, panchine, vialetti
  fillPark(A){
    const T=this.city.TS, X=A.tx*T, Y=A.ty*T, Wp=A.tw*T, Hp=A.th*T;
    const W=this.city.W;
    for(let ty=A.ty;ty<A.ty+A.th;ty++)for(let tx=A.tx;tx<A.tx+A.tw;tx++){
      if(W[ty]&&W[ty][tx]){
        const up=(W[ty-1]&&W[ty-1][tx]), dn=(W[ty+1]&&W[ty+1][tx]);
        this.putTile(!up?'wt_edge':(!dn?'wt_edge':'wt_water'), tx*T, ty*T, !up?0:(!dn?180:0), -18.3);
      }
    }
    const R=(a,b)=>Phaser.Math.Between(a,b);
    const free=(x,y)=>{ const tx=Math.floor(x/T), ty=Math.floor(y/T); return !(W[ty]&&W[ty][tx]); };
    for(let i=0;i<Math.floor(Wp*Hp/22000);i++){
      const ox=R(X+60,X+Wp-60), oy=R(Y+60,Y+Hp-60);
      if(!free(ox,oy)) continue;
      const r=Math.random();
      if(r<0.42) this.putProp('up_tree',ox,oy,true);
      else if(r<0.62) this.putProp('up_bush',ox,oy,false);
      else if(r<0.78) this.putProp('up_bench',ox,oy,true);
      else if(r<0.88) this.putProp('up_lamp',ox,oy,false);
      else this.putProp('up_bin',ox,oy,false);
    }
  }
  // OSPEDALE / UFFICI: UN edificio enorme al centro + qualche massa attorno
  fillBigVenue(A,theme){
    const T=this.city.TS;
    const bw=Math.max(8,Math.floor(A.tw*0.62)), bh=Math.max(6,Math.floor(A.th*0.66));
    const bx=A.tx+Math.floor((A.tw-bw)/2), by=A.ty+Math.floor((A.th-bh)/2);
    this._bldDistrict=A.kind; this._bldTheme=theme;
    this.buildWallRing(bx*T,by*T,bw*T,bh*T,null);
  ...
  planCity(){
    ...
    // ---- ambientazioni: 3 diverse. ospedale/uffici solo nel GRANDE, negozi solo nei piccoli
    const BIG=['ospedale','uffici','parco','residenza'];
    const SML=['negozio','parco','residenza'];
    const big=Phaser.Utils.Array.GetRandom(BIG);
    const pool=Phaser.Utils.Array.Shuffle(SML.slice()).filter(a=>a!==big);
    const s1=pool[0]||'residenza';
    const s2=pool.find(a=>a!==s1)||(s1==='parco'?'residenza':'parco');
    this.sectors=[big,s1,s2];

    // ---- LE DUE STRADE (larghe 2 tessere = una carreggiata per senso) ----
    const hy=Math.round(NY*3/7);        // orizzontale a 3/7 dall'alto
    const vx=Math.round(NX/2);          // verticale al centro, solo sotto
    const road=(x,y)=>{ if(x>=0&&y>=0&&x<NX&&y<NY) R[y][x]=true; };
    for(let x=0;x<NX;x++){ road(x,hy); road(x,hy+1); }
    for(let y=hy;y<NY;y++){ road(vx,y); road(vx+1,y); }

    // ---- I TRE SETTORI ----
    for(let y=0;y<NY;y++)for(let x=0;x<NX;x++){
      if(R[y][x]) { D[y][x]=-1; continue; }        // strada
      D[y][x] = (y<hy) ? 0 : ((x<vx)?1:2);
    }
    // rettangoli utili di ogni settore (in tessere), con un margine dalla strada
    const M=1;
    const areas=[
      {i:0,tx:M,ty:M,tw:NX-M*2,th:hy-M*2},
      {i:1,tx:M,ty:hy+2+M,tw:vx-M*2,th:NY-hy-2-M*2},
      {i:2,tx:vx+2+M,ty:hy+2+M,tw:NX-vx-2-M*2,th:NY-hy-2-M*2}
    ].map(a=>({...a,kind:this.sectors[a.i]}));

    // ---- ACQUA: lago dentro il settore parco ----
    const bridge=[];
    areas.forEach(A=>{
      if(A.kind!=='parco') return;
      const cx=A.tx+Math.floor(A.tw/2), cy=A.ty+Math.floor(A.th/2);
      const rx=Math.max(3,Math.floor(A.tw*0.22)), ry=Math.max(2,Math.floor(A.th*0.22));
      for(let y=cy-ry;y<=cy+ry;y++)for(let x=cx-rx;x<=cx+rx;x++){
        if(x<A.tx||y<A.ty||x>=A.tx+A.tw||y>=A.ty+A.th) continue;
        const dx=(x-cx)/rx, dy=(y-cy)/ry;
        if(dx*dx+dy*dy<=1) W[y][x]=true;
      }
    });

    this.city={TS,NX,NY,R,W,D,areas,blocks:[],bridge,sectors:this.sectors,hy,vx};
    return this.city;
  }

  planMegas(){
    this.megas=[]; this.megaLots={};
    if(this.city&&this.city.areas) return;   // v3: gli edifici li piazza fillSectors
  ...
  buildCity(){
    // v3: la citta' e' fatta di 3 GRANDI SUPERFICI riempite da fillSectors().
    if(this.city && this.city.areas){ this.fillSectors(); return; }
    ...
    for(const _b of _free){
      ...
      // APERTO = ci si entra (stanze+arredi) · BLOCCO = massa chiusa che da' volume
      if(_b.kind==='parco')         tipo = r<0.72?'parco':(r<0.92?'piazza':'aperto');
      else if(_b.kind==='ospedale') tipo = r<0.55?'aperto':(r<0.85?'blocco':'piazza');
      else if(_b.kind==='uffici')   tipo = r<0.45?'aperto':(r<0.9?'blocco':'piazza');
      else if(_b.kind==='negozio')  tipo = r<0.5?'aperto':(r<0.9?'blocco':'piazza');
      else                          tipo = r<0.45?'aperto':(r<0.85?'blocco':'parco');
      ...
      if(tipo==='complesso'){
        ...
        else this.closedMass(px,py,maxW,bodyH,edge);   // troppo piccolo per entrarci: massa chiusa
      ...
      // BLOCCO CHIUSO: massa compatta (non ci si entra), con vicolo interno
      this.closedMass(px,py,maxW,maxH,edge);
  ...
  mkLoot(x,y,type,payload,tex,airdrop){
    ...
    if(airdrop){
      const label=this.add.text(x,y-24,'★ '+WEAPONS[payload].name.toUpperCase(),{fontFamily:TITLE_FONT,fontSize:'13px',fontStyle:'900',color:'#ffc247'}).setOrigin(0.5).setDepth(3).setShadow(0,1,'#000',4);
  ...
  buildHUD(){
    ...
    this.swapTxt=this.add.text(swx,swy+46,'',{fontFamily:TITLE_FONT,fontSize:'13px',color:'#ffd88a',fontStyle:'900'}).setOrigin(0.5).setScrollFactor(0).setDepth(181).setVisible(false);
  ...
  showResults(win,r){
    ...
    if(r.bonus>0) this.add.text(cx,H*0.585,'+'+r.bonus+' bonus sfide!',{fontSize:'13px',fontStyle:'800',color:'#ffc247'}).setOrigin(0.5).setScrollFactor(0).setDepth(301);
    ...
    if(r.wasChallenge && Profile.data._lastResult){
      this.add.text(cx,H*0.72,'⚔ SFIDA · dai questo codice al tuo avversario',{fontSize:'13px',color:'#ff3355',fontStyle:'900'}).setOrigin(0.5).setScrollFactor(0).setDepth(302);
